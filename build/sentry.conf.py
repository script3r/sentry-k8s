...
# handle the case of redis-sentinel
if redis_port == '26379':
    from redis.sentinel import Sentinel

    redis_master = env('SENTRY_REDIS_MASTER') or 'mymaster'
    sentinel = Sentinel([(redis, redis_port),], socket_timeout=0.1)
    redis_host = sentinel.discover_master(redis_master)

    if not redis_host:
        raise Exception('Error: could not obtain redis master from sentinel')

    # select the primary
    redis = redis_host[0]
    redis_port = redis_host[1]
...
SENTRY_WEB_PORT = int(env('SENTRY_WEB_PORT', 9000))
SENTRY_WEB_OPTIONS = {
    'workers':  int(env('SENTRY_WEB_WORKERS', 3)),  # the number of web workers
}
